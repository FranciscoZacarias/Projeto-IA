...
;;; puzzle.lisp Codigo relacionado com o problema
...
(defun no-heuristica (jogo)
"Retorna a heuristica do no jogo"
	(fourth jogo)
)

(defun no-pai (jogo)
"Retorna o no pai do jogo. Retorna nil se nao tiver pai"
	(fifth jogo)
)

(defun no-custo (jogo)
"Devolve o custo de um no"
	(+ (no-profundidade jogo) (no-heuristica jogo))
)
...
(defun alisa-aux (x l)
"Funcao auxiliar de alisa"
	(cond
		((null x) l)
		((equal (type-of (car x)) 'CONS) (alisa-aux (append (car x) (cdr x)) l))
		(t (alisa-aux (cdr x) (append l (list (car x)))))		
	)
)

(defun alisa (lista)
"Retorna a lista sem sublistas"
	(alisa-aux lista ())
)
...
(defun novo-sucessor (operador celula peca jogo heuristica)
"Funcao auxiliar para sucessores.
	...
	(let ((sucessor (funcall operador celula peca jogo)))
		(cond
			...
			(t
				(append
					sucessor
					(list (1+ (no-profundidade jogo)))
					(if (null heuristica) nil (list (funcall heuristica jogo)))
					(list jogo)
				)
			)
		)
...
(defun sucessores-help (jogo operador celulas heuristica)
"Funcao auxiliar de sucessores. Coloca todos os novos sucessores na mesma lista"
	(cond
		((null celulas) nil)
		(t (append
			(mapcar #'(lambda (peca-reserva)
						(funcall 'novo-sucessor operador (car celulas) peca-reserva jogo heuristica) 
				)(reserva jogo)
			)
			(sucessores-help jogo operador (cdr celulas) heuristica)
		))
	)
)

(defun sucessores (jogo operador algoritmo &optional profundidade (heuristica nil))
"Retorna uma lista com todos os sucessores de um estado com base no operador."
	(if (and (equal algoritmo 'dfs) (>= (no-profundidade jogo) profundidade))
		nil
		(funcall 'sucessores-help jogo operador (funcall 'tabuleiro-celulas-vazias (tabuleiro jogo)) heuristica)
	)
)
...
(defun heuristica-base (jogo)
"Calcula o valor heuristico para o no jogo.
Heuristica aplicada: h(x) = 4 - p(x), em que p(x) e a funcao heuristica-base-p"
	(- 4 (funcall 'heuristica-base-p jogo))
)

(defun heuristica-nova-p (jogo)
"Retorna o numero de linhas direcoes validas no tabuleiro que tem 3 pecas.
Se tiver 4 ignora, pois neste ponto ja sabemos que nao e solucao"
	(let* ((listas-tabuleiro (funcall 'tabuleiro-listas (funcall 'tabuleiro jogo)))
			(pecas-por-linha (mapcar #'(lambda (lista)
				(length (remove 0 lista))
			)listas-tabuleiro)))

		(+ (funcall 'conta-ocorrencia 2 pecas-por-linha) (funcall 'conta-ocorrencia 3 pecas-por-linha))
	)
)

(defun heuristica-nova (jogo)
"Calcula o valor heuristico para o no jogo.
Heuristica aplicada: h(x) = 4 - p(x), em que p(x) e a funcao heuristica-nova-p"
	(- 10 (funcall 'heuristica-nova-p jogo))
)

;;; #ENDREGION: Heuristicas